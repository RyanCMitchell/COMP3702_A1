...
public class VisualHelper extends JPanel implements KeyListener {
	...
	private List<ArrayList<Point2D.Double>> linkedPoints = new ArrayList<ArrayList<Point2D.Double>>();
	 ...
	 * 
	 * @param input
	 *            List of Point2D, the vertices
	 */
	public void addLinkedPoints(List<Point2D.Double> input) {
		linkedPoints.add(new ArrayList<Point2D.Double>());
	...
	/**
	 * Waits until user presses a key, then returns the key pressed Will return
	 * 0 if timeout occurs
	 * 
	 * @param timeout
	...
	public void paint(Graphics g) {
		...
		for (int i = 0; i < linkedPoints.size(); i++) {
			...
			for (int j = 0; j < linkedPoints.get(i).size() - 1; j++) {
				Point2D.Double p1 = linkedPoints.get(i).get(j);
				Point2D.Double p2 = linkedPoints.get(i).get(j + 1);
